...
//   accessor: keyof ExpiresTime;
//   label: string;
//   digitCount: number;
// }

// interface ExpiresTime {
//   daysLeft: number;
//   hoursLeft: number;
//   minutesLeft: number;
//   secondsLeft: number;
...
//   { type: 'seconds', accessor: 'secondsLeft', label: 'sec', digitCount: 2 },
//   { type: 'minutes', accessor: 'minutesLeft', label: 'min', digitCount: 2 },
//   { type: 'hours', accessor: 'hoursLeft', label: 'hour', digitCount: 2 },
//   { type: 'days', accessor: 'daysLeft', label: 'day', digitCount: 3 },
// ];

// const FlipTimer: React.FC<FlipTimerProps> = ({ endDate, onComplete, auctionData, defaultTime }) => {
//   const [expiresTime, setExpiresTime] = useState<ExpiresTime>({
//     daysLeft: 0,
//     hoursLeft: 0,
//     minutesLeft: 0,
//     secondsLeft: 0,
...
//   const prevExpiresTimeRef = useRef<ExpiresTime>(expiresTime); // Добавляем ref для предыдущего состояния

//   const wrapperRef = useRef<HTMLDivElement>(null);

//   // Определяем, показывать ли таймер и какое значение использовать
...
//   const calculateExpiresTime = useCallback((endDate: Date): ExpiresTime => {
...
//     const dateDifference = Math.abs(todayDate.getTime() - endDate.getTime());

//     return {
//       daysLeft: Math.max(0, Math.floor(dateDifference / ONE_DAY)),
//       hoursLeft: Math.max(0, Math.floor((dateDifference / ONE_HOUR) % 24)),
//       minutesLeft: Math.max(0, Math.floor((dateDifference / ONE_MINUTE) % 60)),
//       secondsLeft: Math.max(0, Math.floor((dateDifference / ONE_SECOND) % 60)),
//     };
//   }, []);

//   // Преобразуем секунды в ExpiresTime
//   const convertSecondsToExpiresTime = useCallback((seconds: number): ExpiresTime => {
//     const ONE_MINUTE = 60;
//     const ONE_HOUR = ONE_MINUTE * 60;
//     const ONE_DAY = ONE_HOUR * 24;

//     return {
//       daysLeft: Math.floor(seconds / ONE_DAY),
//       hoursLeft: Math.floor((seconds % ONE_DAY) / ONE_HOUR),
//       minutesLeft: Math.floor((seconds % ONE_HOUR) / ONE_MINUTE),
//       secondsLeft: Math.floor(seconds % ONE_MINUTE),
//     };
//   }, []);

//   useEffect(() => {
//     // Если аукцион еще не начат, используем defaultTime
//     if (isAuctionNotStarted && defaultTime !== undefined) {
//       const staticTime = convertSecondsToExpiresTime(defaultTime);
//       setExpiresTime(staticTime);
//       prevExpiresTimeRef.current = staticTime;
//       return;
//     }

//     if (!timerEndDate) return;

//     const parsedEndDate = timerEndDate instanceof Date ? timerEndDate : new Date(timerEndDate);
...
//         const newExpiresTime = calculateExpiresTime(parsedEndDate);
//         const newFlippingDigits: { [key: string]: boolean } = {};

//         TIMER_CONFIG.forEach(({ type, accessor, digitCount }) => {
//           const currentValue = newExpiresTime[accessor].toString().padStart(digitCount, '0');
//           const prevValue = prevExpiresTimeRef.current[accessor].toString().padStart(digitCount, '0');

//           if (currentValue !== prevValue) {
//             newFlippingDigits[type] = true;
//           }
//         });

//         setExpiresTime(newExpiresTime);
//         prevExpiresTimeRef.current = newExpiresTime; // Обновляем ref
//         setFlippingDigits(newFlippingDigits);
//         lastUpdateRef.current = now;

//         // Проверка на завершение таймера
//         if (
//           newExpiresTime.daysLeft === 0 &&
//           newExpiresTime.hoursLeft === 0 &&
//           newExpiresTime.minutesLeft === 0 &&
//           newExpiresTime.secondsLeft === 0
//         ) {
...
//     // Первый запуск
//     const initialTime = calculateExpiresTime(parsedEndDate);
//     setExpiresTime(initialTime);
//     prevExpiresTimeRef.current = initialTime;
...
//       }
//     };
//   }, [timerEndDate, onComplete, isAuctionNotStarted, defaultTime, calculateExpiresTime, convertSecondsToExpiresTime]);

//   const renderDigits = useCallback((type: string, value: number, digitCount: number, showInfinity: boolean = false) => {
//     if (showInfinity) {
//       return (
//         <>
...
//           </div>
//           <div className="flip-digit-label">{TIMER_CONFIG.find(c => c.type === type)?.label}</div>
//         </>
//       );
//     }

//     const strValue = value.toString().padStart(digitCount, '0');
//     const prevStrValue = prevExpiresTimeRef.current[type as keyof ExpiresTime]?.toString().padStart(digitCount, '0') || '0'.repeat(digitCount);
...
//           </Typography>
//           <ul 
//             className="flip-clock-container show" 
//             id="flip-clock-container"
//           >
//             {TIMER_CONFIG.map(({ type, accessor, digitCount }) => (
//               <li key={type} className={`flip-item-${type}`}>
//                 {renderDigits(type, expiresTime[accessor], digitCount, true)}
//               </li>
//             ))}
//           </ul>
...
//             {TIMER_CONFIG.map(({ type, accessor, digitCount }) => (
//               <li key={type} className={`flip-item-${type}`}>
//                 {renderDigits(type, expiresTime[accessor], digitCount, false)}
//               </li>
//             ))}
//           </ul>
//         </div>
...
//   // Если нет данных аукциона, показываем обычный таймер или ничего
