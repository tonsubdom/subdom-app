...
// src/components/AuctionCollectionSelector/CustomZoneSelector.tsx
// ИСПРАВЛЕНИЯ (v2):
// 1) getZoneSubdomainCount для proxy использует zone.subdomainsAmount в первую очередь
// 2) getZoneSubdomainCount для sbt использует zone.subdomainsAmount если нет sbtZonesCount
// 3) Фильтр пустых имён: ".", ".ton", "*.ton" и т.д. — добавлен в filteredAndSortedZones

// src/components/AuctionCollectionSelector/CustomZoneSelector.tsx
// v2 — БЕЗ БЭКЕНДА
// Количество субдоменов берётся ТОЛЬКО из zone.subdomainsAmount (ончейн item_count)
// Фильтр мусорных зон: пустые, "*", "pseudonym"
...
const getItemCountColor = (count: number): string => {
  ...
  if (count >= 1 && count <= 10) return "#10b981";
  if (count >= 11 && count <= 50) return "#3b82f6";
  if (count >= 51 && count <= 250) return "#8b5cf6";
  if (count >= 251 && count <= 500) return "#f59e0b";
  return "#f97316";
};
...
export const CustomZoneSelector: React.FC<CustomZoneSelectorProps> = ({
  ...
  // ====== КОЛИЧЕСТВО СУБДОМЕНОВ — ТОЛЬКО ИЗ ОНЧЕЙН ======
  const getZoneSubdomainCount = (zone: Zone): number => {
    return zone.subdomainsAmount || 0;
  };
  ...
  const getZoneDate = (zone: Zone): number => {
    ...
    return typeof zone.id === "number" ? zone.id : 0;
  };

  // ====== ФИЛЬТРАЦИЯ (мусорные зоны) + СОРТИРОВКА ======
  const filteredAndSortedZones = useMemo(() => {
    let result = zones.filter((zone) => {
      const name = zone.name || "";
      const zonePart = formatZoneName(name);
      // Отсеиваем пустые, "*", "pseudonym"
      if (!zonePart || zonePart.length === 0) return false;
      if (zonePart === "*") return false;
      if (zonePart === "pseudonym") return false;
      return true;
    });
    ...
    return result;
  }, [zones, searchQuery, showOnlyMyZones, userAddress, sortState]);

  const selectedZoneInfo = useMemo(() => {
    if (!selectedZone) return null;
    const zone = zones.find((z) => z.name === selectedZone);
    if (!zone) return null;
    const itemCount = getZoneSubdomainCount(zone);
    return {
      ...zone,
      itemCount,
      color: getItemCountColor(itemCount),
  ...
  const totalSubdomains = useMemo(() => {
    return zones.reduce((sum, z) => sum + (z.subdomainsAmount || 0), 0);
  }, [zones]);
  ...
  const handleSortToggle = (type: SortType) => {
    setSortState((prev) => {
      if (prev.type === type) {
        return { type, direction: prev.direction === "asc" ? "desc" : "asc" };
      }
          ...
          >
            {filteredAndSortedZones.length === 0 ? (
              ...
              filteredAndSortedZones.map((zone) => {
                ...
                const { itemCount, percentage } = zoneDataItem;
                const isUserZone = zone.owner === userAddress;
