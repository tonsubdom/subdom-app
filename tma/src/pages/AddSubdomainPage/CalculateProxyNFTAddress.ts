...
/**
 * Получает информацию об адресе с использованием API ключа
 */
...
// ): Promise<string | null> => {
//   try {
//     console.log(`🔍 Расчет адреса NFT для Proxy субдомена: ${subdomainName} в коллекции: ${collectionAddress}`);
    
//     // 1. Вычисляем индекс субдомена
//     const index = getDnsItemIndex(subdomainName);
//     console.log(`Subdomain index: ${index.toString()}`);
    
//     // 2. Получаем адрес NFT через метод коллекции
//     const nftAddress = await getNFTAddressByIndex(index.toString(), collectionAddress, isTestnet);

//     console.log(`Результат НФТ-адреса с помощью index:${nftAddress}`);
    
//     if (nftAddress) {
//       console.log(`✅ Получен корректный адрес NFT: ${nftAddress}`);
//       return nftAddress;
//     }
    
//     // 3. Если не удалось получить через API, используем детерминированный расчет
//     console.log('⚠️ Используем детерминированный расчет адреса NFT для Proxy');
    
//     // Правильный детерминированный расчет адреса NFT для TON
//     // Формула: адрес NFT = hash(collection_address + index)
//     const calculatedAddress = await calculateDeterministicNFTAddress(index.toString(), collectionAddress);
    
//     console.log(`📝 Расчетный адрес NFT: ${calculatedAddress}`);
//     return calculatedAddress;
    
//   } catch (error) {
//     console.error('Error calculating Proxy NFT address:', error);
//     return null;
//   }
// };

// /**
//  * Получает адрес NFT по индексу через метод коллекции
//  */
// const getNFTAddressByIndex = async (
//   index: string,
//   collectionAddress: string,
...
// // /**
// //  * Детерминированный расчет адреса NFT для TON
// //  * Использует правильную формулу расчета адреса NFT в TON
// //  */
// // const calculateDeterministicNFTAddress = async (
// //   index: string,
// //   collectionAddress: string
// // ): Promise<string> => {
// //   try {
// //     // Для TON NFT стандарт использует:
// //     // NFT адрес = StateInit hash контракта NFT
    
// //     // В реальном проекте используйте библиотеку ton или tonweb для правильного расчета
// //     // Пример с использованием ton-core:
    
// //     // Упрощенный расчет для демонстрации
// //     // В реальности нужно использовать правильный StateInit
    
// //     // Используем хеш от конкатенации коллекции и индекса
// //     const message = `${collectionAddress}:${index}`;
// //     const hash = await sha256(message);
    
// //     // Формируем адрес в правильном формате
// //     // Базовый адрес: первые 48 символов коллекции + хеш
// //     const cleanCollectionAddr = collectionAddress.replace(/:/g, '').slice(0, 48);
// //     const cleanHash = hash.slice(0, 16);
    
// //     // Формат: EQ... (базовый адрес TON)
// //     const calculatedAddress = `${cleanCollectionAddr}${cleanHash}`;
...
//     // Парсим адрес коллекции
//     const collectionAddr = Address.parse(collectionAddress);
    
//     // Создаем StateInit для NFT
//     // В реальном проекте нужно использовать правильный код и данные для NFT контракта
//     // Это упрощенный пример для стандартного NFT контракта
    
//     // Создаем ячейку с данными для NFT
//     const dataCell = beginCell()
//       .storeUint(0, 1) // init
//       .storeUint(BigInt(index), 64) // index
//       .storeAddress(collectionAddr) // collection address
//       .storeAddress(null) // owner address (null для нового NFT)
//       .storeCoins(0) // individual content
//       .endCell();
    
//     // Создаем StateInit
//     const stateInit: StateInit = {
//       code: await getStandardNFTCode(), // Получаем стандартный код NFT
//       data: dataCell
//     };
    
//     // Сериализуем StateInit в ячейку
...
//     // Создаем адрес на основе хеша StateInit
//     const nftAddr = new Address(0, stateInitHash);
    
//     // Возвращаем адрес в строковом формате
//     return nftAddr.toString({ bounceable: true, testOnly: false });
    
//   } catch (error) {
//     console.error('Error in deterministic calculation:', error);
//     // Fallback: альтернативный расчет
//     return await calculateDeterministicNFTAddressFallback(index, collectionAddress);
//   }
// };

// /**
//  * Альтернативный метод расчета адреса NFT (fallback)
//  */
// const calculateDeterministicNFTAddressFallback = async (
//   index: string,
//   collectionAddress: string
// ): Promise<string> => {
//   try {
//     // Используем стандартную формулу TON для расчета адреса контракта
//     // Адрес = hash(workchain + state_init_hash)
    
//     const collectionAddr = Address.parse(collectionAddress);
    
//     // Создаем уникальный идентификатор для NFT
//     const uniqueId = `${collectionAddr.toString()}:${index}`;
    
//     // Хешируем идентификатор
//     const hash = await sha256(uniqueId);
    
//     // Создаем адрес на основе хеша
//     // Используем workchain 0 (базовый)
//     const hashBuffer = Buffer.from(hash, 'hex');
//     const addressHash = hashBuffer.slice(0, 32); // 32 байта для адреса
    
//     const nftAddr = new Address(0, addressHash);
    
//     return nftAddr.toString({ bounceable: true, testOnly: false });
    
//   } catch (error) {
//     console.error('Error in fallback calculation:', error);
//     // Последний fallback: простой формат
//     return `${collectionAddress}:${index}`;
//   }
// };

// /**
//  * Получает стандартный код NFT контракта
//  * В реальном проекте нужно использовать актуальный код NFT контракта
//  */
// const getStandardNFTCode = async (): Promise<Cell> => {
//   // Это упрощенный пример. В реальном проекте нужно:
//   // 1. Использовать актуальный код NFT контракта из репозитория TON
//   // 2. Или загружать его из сети
  
//   // Создаем пустую ячейку для демонстрации
//   // В реальном проекте здесь должен быть реальный код контракта
//   return beginCell()
//     .storeUint(0xFFFFFFFF, 32) // placeholder
//     .endCell();
// };
...
//   // Проверяем базовые признаки валидного адреса TON
//   const tonAddressRegex = /^(?:-1|0):[0-9a-fA-F]{64}$/;
//   const eqAddressRegex = /^EQ[0-9a-zA-Z_-]{48}$/;
...
//     // 1. Сначала пытаемся получить информацию об аукционе
//     // (импортируйте getAuctionInfo из соответствующего файла)
//     // const auctionInfo = await getAuctionInfo(subdomainName, collectionAddress, isTestnet);
    
//     // 2. Если аукцион не найден, рассчитываем адрес NFT
//     const nftAddress = await calculateProxyNFTAddress(subdomainName, collectionAddress, isTestnet);
...
//     // 3. Проверяем, существует ли уже NFT
