...
//     // 1. Вычисляем индекс субдомена (используем ту же функцию что и в аукционах)
//     const index = getDnsItemIndex(subdomainName);
//     console.log(`Subdomain index: ${index.toString()}`);
    
//     // 2. Получаем адрес NFT по индексу
//     const nftAddress = await getNFTAddressByIndex(index.toString(), collectionAddress, isTestnet);
    
//     if (!nftAddress) {
//       // NFT не найден - субдомен доступен
//       console.log('✅ SBT subdomain is available (no NFT found)');
//       return {
//         isTaken: false
//       };
//     }
...
//       };
//     } else {
//       // NFT не активен или не существует
...
// /**
//  * Получает информацию об адресе (включая владельца)
//  */
...
//       return {
//         isTaken: false,
//         nftAddress: await calculateNFTAddress(index.toString(), collectionAddress, isTestnet) // ВАЖНО: возвращаем рассчитанный адрес
//       };
//     }
...
//       return {
//         isTaken: false,
//         nftAddress: nftAddress // ВАЖНО: возвращаем адрес даже если NFT не активен
//       };
//     }
...
// /**
//  * Рассчитывает адрес NFT по индексу (без запроса к API)
//  */
...
// ): Promise<string | null> => {
//   try {
//     // Используем ту же логику что и в getNFTAddressByIndex, но без запроса
//     // Это расчетный адрес, который будет создан при минте
//     const apiUrl = isTestnet
//       ? "https://testnet.toncenter.com/api/v2/runGetMethod"
//       : "https://toncenter.com/api/v2/runGetMethod";
...
//     // Если не удалось получить через API, рассчитываем локально
//     // Это упрощенный расчет - в реальности нужно использовать правильную формулу
//     console.log('⚠️ Используем расчетный адрес NFT');
...
// export interface SBTSubdomainInfo {
//   isTaken: boolean;
//   nftAddress?: string; // Может быть undefined
//   ownerAddress?: string;
//   timestamp?: number;
// }
...
//       // NFT не найден - субдомен доступен
//       console.log('✅ SBT subdomain is available (no NFT found)');
//       const calculatedAddress = await calculateNFTAddress(index.toString(), collectionAddress, isTestnet);
//       return {
//         isTaken: false,
//         nftAddress: calculatedAddress || undefined // Преобразуем null в undefined
//       };
//     }
...
//       return {
//         isTaken: false,
//         nftAddress // Возвращаем адрес даже если NFT не активен
//       };
//     }
