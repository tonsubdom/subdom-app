...
// } from "@/services/blockchainItems/blockchain-items-slice";
// import { Zone } from "@/services/api";
// import { CustomZoneSelector } from "./CustonZoneSelector";
// import { getUserSbtSubdomainsCount } from "@/utils/sbt-utils";
...
//   AuctionCollectionSelectorProps
// > = ({
//   activeTab,
//   selectedDomainZone,
//   onDomainZoneChange,
...
//       // Используем новую утилиту для получения количества SBT субдоменов
//       const userSbtCounts = await getUserSbtSubdomainsCount(
//         userAddress,
//         isTestnet
//       );
...
//   const getCollectionAddress = (zoneName: string): string | null => {
//     if (activeTab === "proxy") {
//       // 1. Ищем в переданных proxyZones (из БД)
//       const zoneFromDb = proxyZones.find((z) => z.name === zoneName);
//       if (zoneFromDb?.collectionAddress) {
//         return zoneFromDb.collectionAddress;
//       }
//       // 2. Fallback: ТОЧНОЕ совпадение в Redux proxyCollections
//       const collection = proxyCollections.find((c) => c.name === zoneName);
//       return collection?.address || null;
//     } else {
//       // SBT: ТОЧНОЕ совпадение в Redux sbtCollections
//       const collection = sbtCollections.find((c) => c.name === zoneName);
//       return collection?.address || null;
//     }
//   };
...
//     if (collectionAddress) {
//       console.log(
//         `Selected zone: ${zoneName}, collection: ${collectionAddress}`
//       );
//     }
//   };
...
//         mode={activeTab}
//         loadSbtSubdomains={activeTab === "sbt" ? loadSbtSubdomains : undefined}
//         sbtZonesCount={activeTab === "sbt" ? sbtZonesCount : undefined}
//       />
...
//     if (activeTab === "proxy") {
//       // 1. Ищем в proxyZones из БД
//       const zoneFromDb = proxyZones.find((z) => z.name === selectedDomainZone);
//       if (zoneFromDb?.collectionAddress) {
//         return zoneFromDb.collectionAddress;
//       }
//       // 2. Fallback: ТОЧНОЕ совпадение в Redux
//       const collection = proxyCollections.find(
//         (c) => c.name === selectedDomainZone
//       );
//       return collection?.address || null;
//     } else {
//       // SBT
...
//         return zoneFromDb.collectionAddress;
//       }
//       const collection = sbtCollections.find(
//         (c) => c.name === selectedDomainZone
//       );
//       return collection?.address || null;
//     }
...
> = ({
  ...
  activeSbtZones = [],
  proxyZones = [],
  // isTestnet больше не используется внутри компонента
  // sbtZonesCount больше не используется — количество берётся из zone.subdomainsAmount
}) => {
  const proxyCollections = useSelector(selectProxyCollections);
  const sbtCollections = useSelector(selectSBTCollections);

  // Зоны в зависимости от activeTab
  const zones = useMemo(() => {
    return activeTab === "proxy" ? proxyZones : activeSbtZones;
  }, [activeTab, proxyZones, activeSbtZones]);
  ...
  // Placeholder
  const placeholder = useMemo(() => {
    if (zonesLoading) return t("loadingZones");
    if (zonesError) return t("zonesLoadError");
    return activeTab === "proxy" ? t("chooseProxyZone") : t("chooseSbtZone");
  }, [zonesLoading, zonesError, activeTab, t]);

  const hasZones = zones && zones.length > 0;

  // Получение collectionAddress
  const getCollectionAddress = (zoneName: string): string | null => {
    if (activeTab === "proxy") {
      const zoneFromDb = proxyZones.find((z) => z.name === zoneName);
      if (zoneFromDb?.collectionAddress) return zoneFromDb.collectionAddress;
      const collection = proxyCollections.find((c) => c.name === zoneName);
      return collection?.address || null;
    } else {
      const collection = sbtCollections.find((c) => c.name === zoneName);
