...
export class UniversalBlockchainService {
  ...
  //   try {
  //     // 1. Получаем все коллекции от владельца платформы
  //     const allCollections = await this.getAllCollectionsFromPlatformOwner();
  //     console.log(`✅ Получено ${allCollections.length} коллекций от владельца платформы`);
      
  //     // 2. Классифицируем коллекции
  //     const proxyCollections = allCollections.filter(c => this.classifier.isProxyCollection(c));
  //     const sbtCollections = allCollections.filter(c => this.classifier.isSBTCollection(c));
      
  //     console.log(`📊 Коллекции: ${proxyCollections.length} proxy, ${sbtCollections.length} sbt`);
      
  //     // 3. Получаем NFT Wrapper коллекцию (известна заранее)
  //     const nftWrapperCollection = await this.getNFTWrapperCollection();
  //     const nftWrapperCollections = nftWrapperCollection ? [nftWrapperCollection] : [];
      
  //     console.log(`📊 NFT Wrapper коллекций: ${nftWrapperCollections.length}`);
      
  //     // 4. Получаем все итемы с учетом логики
  //     const allItems: TonCenterNFTItem[] = [];
      
  //     // Proxy коллекции - все итемы
  //     if (this.config.loadProxySubdomains && proxyCollections.length > 0) {
  //       console.log('📦 Загрузка всех proxy субдоменов...');
  //       const proxyItems = await this.getAllItemsFromCollections(proxyCollections);
  //       allItems.push(...proxyItems);
  //       console.log(`✅ Загружено ${proxyItems.length} proxy субдоменов`);
  //     }
      
  //     // SBT коллекции - только итемы пользователя (если указан)
  //     if (this.config.loadSBTSubdomains && sbtCollections.length > 0 && userAddress) {
  //       console.log('📦 Загрузка SBT субдоменов пользователя...');
  //       const sbtItems = await this.getUserItemsFromCollections(sbtCollections, userAddress);
  //       allItems.push(...sbtItems);
  //       console.log(`✅ Загружено ${sbtItems.length} SBT субдоменов пользователя`);
  //     }
      
  //     // NFT Wrapper коллекция - все итемы
  //     if (this.config.loadNFTWrappers && nftWrapperCollections.length > 0) {
  //       console.log('📦 Загрузка всех NFT оберток...');
  //       const wrapperItems = await this.getAllItemsFromCollections(nftWrapperCollections);
  //       allItems.push(...wrapperItems);
  //       console.log(`✅ Загружено ${wrapperItems.length} NFT оберток`);
  //     }
      
  //     console.log(`📊 Всего итемов: ${allItems.length}`);
      
  //     // 5. Классифицируем итемы
  //     const proxySubdomains = allItems.filter(item => this.classifier.isProxySubdomain(item));
  //     const sbtSubdomains = allItems.filter(item => this.classifier.isSBTSubdomain(item));
  //     const nftWrappers = allItems.filter(item => this.classifier.isNFTWrapper(item));
      
  //     console.log(`📊 Итемы: ${proxySubdomains.length} proxy, ${sbtSubdomains.length} sbt, ${nftWrappers.length} nft wrapper`);
      
  //     // 6. Получаем метаданные
  //     const metadata = await this.getMetadataForAll([...allItems, ...allCollections, ...nftWrapperCollections]);
      
  //     // 7. Конвертируем в простые типы
  ...
  //     const simpleNFTWrapperCollections = nftWrapperCollections.length > 0 
  //       ? convertToSimpleCollections(nftWrapperCollections, metadata, this.isTestnet)
  //       : [];
      
  //     const simpleAllItems = convertToSimpleEnrichedItems(allItems, metadata, this.isTestnet);
  //     const simpleProxySubdomains = simpleAllItems.filter(item => item.type === 'proxy_subdomain');
  //     const simpleSBTSubdomains = simpleAllItems.filter(item => item.type === 'sbt_subdomain');
  //     const simpleNFTWrappers = simpleAllItems.filter(item => item.type === 'nft_wrapper');
      
  //     // 8. Фильтруем по пользователю если указан адрес
  //     let userProxySubdomains: SimpleEnrichedItem[] = [];
  //     let userSBTSubdomains: SimpleEnrichedItem[] = [];
  //     let userNFTWrappers: SimpleEnrichedItem[] = [];
      
  //     if (userAddress) {
  //       // Для proxy субдоменов фильтруем из всех
  //       userProxySubdomains = simpleProxySubdomains.filter(item => item.owner_address === userAddress);
        
  //       // Для SBT субдоменов уже загружены только пользовательские
  //       userSBTSubdomains = simpleSBTSubdomains;
        
  //       // Для NFT оберток фильтруем из всех
  //       userNFTWrappers = simpleNFTWrappers.filter(item => item.owner_address === userAddress);
        
  //       console.log(`👤 Итемы пользователя: ${userProxySubdomains.length} proxy, ${userSBTSubdomains.length} sbt, ${userNFTWrappers.length} nft wrapper`);
  //     }
  ...
  //       // Все итемы
  //       allItems: simpleAllItems,
  //       proxySubdomains: simpleProxySubdomains,
  //       sbtSubdomains: simpleSBTSubdomains,
  //       nftWrappers: simpleNFTWrappers,
  ...
  //     // Метаданные
  //     const metadata = await this.getMetadataForAll([...allCollections, ...nftWrapperCollections]);
  ...
  //     // 2. Получаем итемы с учетом логики
  //     const allItems: TonCenterNFTItem[] = [];
  ...
  //     // SBT коллекции - только итемы пользователя (если указан)
  //     if (sbtCollections.length > 0 && userAddress) {
  //       const sbtItems = await this.getUserItemsFromCollections(sbtCollections, userAddress);
  //       allItems.push(...sbtItems);
  //     }
      
  //     // NFT Wrapper коллекция - все итемы
  //     if (nftWrapperCollections.length > 0) {
  //       const wrapperItems = await this.getAllItemsFromCollections(nftWrapperCollections);
  //       allItems.push(...wrapperItems);
  //     }
      
  //     // 3. Метаданные и конвертация
  //     const metadata = await this.getMetadataForAll(allItems);
  //     const simpleAllItems = convertToSimpleEnrichedItems(allItems, metadata, this.isTestnet);
      
  //     const proxySubdomains = simpleAllItems.filter(item => item.type === 'proxy_subdomain');
  //     const sbtSubdomains = simpleAllItems.filter(item => item.type === 'sbt_subdomain');
  //     const nftWrappers = simpleAllItems.filter(item => item.type === 'nft_wrapper');
  ...
  //     this.cache.set(cacheKey, result);
  //     console.log(`✅ Данные итемов загружены: ${simpleAllItems.length} всего`);
  ...
  //     const items = await this.getAllItemsFromCollections([collection]);
  //     const metadata = await this.getMetadataForAll(items);
  //     const result = convertToSimpleEnrichedItems(items, metadata, this.isTestnet);
