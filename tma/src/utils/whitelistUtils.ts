 ...
 */
// export function checkWhitelist(address: string): {
...
//для майннета
// export function checkWhitelist(address: string, isTestnet: boolean = true): {
//   isWhitelisted: boolean;
//   telegramName?: string;
//   testnetAddress?: string;
// } {
//   try {
//     if (isTestnet) {
//       // Testnet: конвертируем в формат 0Q и проверяем whitelistMap
//       const testnetAddress = convertRawToUserFriendlyTest(address);
//       const telegramName = whitelistMap[testnetAddress];
      
//       if (telegramName) {
//         return { isWhitelisted: true, telegramName, testnetAddress };
//       }
//       return { isWhitelisted: false, testnetAddress };
//     } else {
//       // Mainnet: проверяем mainnetWhitelistMap напрямую
//       // Адрес уже в формате UQ/EQ от TonConnect — ищем прямую сверку
//       const telegramName = mainnetWhitelistMap[address];
      
//       if (telegramName) {
//         return { isWhitelisted: true, telegramName, testnetAddress: address };
//       }
//       return { isWhitelisted: false, testnetAddress: address };
//     }
//   } catch (error) {
//     console.error('Ошибка проверки вайтлиста:', error);
...
//       return { isWhitelisted: false, testnetAddress };
//     } else {
//       // Mainnet: конвертируем ВХОДЯЩИЙ адрес в raw (0:...)
//       // Это решает проблему EQ/UQ — raw-формат един для обоих
//       const rawAddress = Address.parse(address).toString(); // даст "0:..."
//       console.log(`Подключенный адрес переведенный в сырую форму:${rawAddress}`);
...
//       console.log('🔍 [checkWhitelist] Mainnet raw:', rawAddress);
//       const telegramName = mainnetWhitelistMap[rawAddress];
//       console.log('🔍 [checkWhitelist] Найден в мапе?', !!telegramName);
