...
const handleAuctionClick = (auction: ActiveAuction) => {
  ...
  // маунт: ensureData меняет идентичность после каждого fetch (appData/lastUpdated
  // в её зависимостях), зависимость от неё самой заставляла этот эффект
  // перезапускаться на каждую загрузку, лишний раз дёргая проверку свежести
  // параллельно с 30-секундным сканом аукционов ниже.
  useEffect(() => {
    ensureData();
  }, []);
