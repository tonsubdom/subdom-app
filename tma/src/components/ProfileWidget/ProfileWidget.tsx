...
const collectionToZone = (col: SimpleCollection): Zone => {
  ...
  return {
    ...
    name: `${zoneName}.ton`,
    // zoneName.endsWith(".ton") ? zoneName : `${zoneName}.ton`,
    address: col.address,
...
const ProfileWidget: React.FC = () => {
  ...
  const renderSubdomainCard = (subdomain: Subdomain) => {
    ...
    // [NEW] URL картинки для swipe
    const imgUri = isSwipe ? getSubdomainImage(subdomain) : undefined;

    // [NEW] Проверка по ончейн-аукционам
