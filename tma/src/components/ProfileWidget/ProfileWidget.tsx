...
const collectionToZone = (col: SimpleCollection): Zone => {
  ...
  // Два РАЗНЫХ источника данных отдают два РАЗНЫХ вида "мусора" для ещё не
  // проиндексированной/только что пересозданной коллекции: бэкенд-кэш
  // (platformCacheClient.ts) отдаёт настоящую пустую строку, а прямой
  // конвертер с toncenter (convertToSimpleCollection в
  // blockchain-items-utils.ts) при пустых метаданных возвращает буквальный
  // литерал 'Без названия' — НЕПУСТУЮ строку, которая раньше не ловилась
  // проверкой на пустоту и вдобавок затирала уже сохранённое хорошее имя в
  // localStorage-кэше (см. rememberZoneName ниже). Оба случая считаем
  // "именем ещё нет" и не даём затереть remembered-кэш.
  ...
  // toncenter с ещё не проиндексированными метаданными — раньше это давало
  // голое ".ton" в карточке (см. пересоздание SBT→Proxy на одном домене).
  // Настоящее имя уже известно СРАЗУ в момент деплоя (см. rememberZoneName
  // в CreateCollectionPage) и лежит в персистентном localStorage-кэше
  // независимо от toncenter — юзер не видит вообще никакого шва, пока
  // индексация не подтянется сама. Адрес как плейсхолдер — только самый
