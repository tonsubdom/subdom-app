...
const isSbtSubdomain = (subdomain: Subdomain) => {
  ...
  marginLeft: '12px'
   ...
   <div>
    {activeTab === 'zones' && (
      <span>
        {t('foundZones')}: <strong style={{ color: colors.primary }}>{getFilteredZones().length}</strong> {t('of ')}{getUserZones.length}
      </span>
    )}
    {activeTab === 'subdomains' && (
      <span>
        {t('foundSubdomains')}: <strong style={{ color: colors.primary }}>{getFilteredSubdomains().length}</strong> {t('of ')}{subdomains.length}
      </span>
    )}
  </div>
                          ...
                          >
                            ...
                            >
                             ...
                             {/* <p
                                ...
                                >
                                  {t('currentBid')}: <span style={{color: colors.cyberpunk, textDecoration: 'none'}}>{(auction.lastBid / 1_000_000_000).toFixed(2)} <img 
                          src={TonLogo} 
                          alt="TON"
                          style={{
                            width: '16px',
  ...
  <div className="scrollPartWrapper" style={{overflow: 'scroll', height: '100%'}}>
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "13px", color: colors.text, opacity: 0.8, fontFamily: 'monospace' }}>
        ...
        {/* Активные аукционы (оставляем как было) */}
